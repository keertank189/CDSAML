...
class Transliterator():
    ...
    def __init__(self, iast=False):
        ...
        self.transliterables = \
                   list(self.CONSONANTS.keys()) + list(self.COMBINING_VOWELS.keys()) + ['']
    ...
    def process_token(self, token):
        ...
        if token == u'\'\u1E41':
            self.devanagari.append(''.join(
                     [chr(int(k)) for k in [self.ANUSVARA, self.AVAGRAHA]]))
            self.state[0] = 0
            return True

        if token in self.CONSONANTS:
            if self.state[0] == 1:
                self.devanagari.append(chr(int(self.VIRAMA)))
            self.devanagari.append(''.join(
                             [chr(int(k)) for k in self.CONSONANTS[token]]))
            self.state[0] = 1
            return True

        if self.state[0] < 1 and token in self.INITIAL_VOWELS:
            self.devanagari.append(''.join(
                         [chr(int(k)) for k in self.INITIAL_VOWELS[token]]))
            return True

        if self.state[0] == 1 and token in self.COMBINING_VOWELS:
            self.devanagari.append(''.join(
                       [chr(int(k)) for k in self.COMBINING_VOWELS[token]]))
            self.state[0] = 0
            return True
